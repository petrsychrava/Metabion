...
public class LabResultService {
    ...
    private User clinicalPatient(Authentication authentication, Long patientId) {
        ...
        if (!accessControl.canViewPatientClinicalData(authentication, patientId)) throw forbidden("Patient profile is not assigned to current user");
        return actor;
    }
    ...
    private void requireClinicalActor(User actor) { if (!actor.hasAnyRole(RoleName.NUTRITION_SPECIALIST, RoleName.PHYSICIAN)) throw forbidden("Current user cannot access clinical data"); }
